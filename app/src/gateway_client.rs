...
use gateway_remote::protocol::{AgentSummary, ClientMessage, PermissionAnswer, PromptInput, ServerMessage};
...
impl GatewayClient {
    ...
    pub fn remote_socket_url(&self, ticket: &str) -> String {
        let base = self.base_url.replace("https://", "wss://").replace("http://", "ws://");
        format!("{}/remote?ticket={}", base.trim_end_matches('/'), encode_path_segment(ticket))
    }
    ...
    async fn decode<T: DeserializeOwned>(&self, response: reqwest::Response) -> Result<T> {
        ...
        if !status.is_success() {
            let message = serde_json::from_str::<serde_json::Value>(&text)
                .ok()
                .and_then(|body| body.get("message").and_then(|value| value.as_str()).map(ToOwned::to_owned))
                .unwrap_or(text);
    ...
    pub async fn begin_pairing(&self) -> Result<PairingOffer> {
        self.post_json("/pairing/begin", serde_json::json!({})).await
    }
    ...
    pub async fn subscribe_message(
        ...
        let (socket, _) = tokio_tungstenite::connect_async(self.session_stream_url(session_id, after_seq)).await?;
        Ok(socket)
    }
    ...
    pub async fn parse_remote_frame(value: serde_json::Value) -> Result<RemoteFrame> {
        if value.get("seq").and_then(|seq| seq.as_u64()).is_some()
            && value.get("session_id").and_then(|session_id| session_id.as_str()).is_some()
        {
