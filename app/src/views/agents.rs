            ...
            div()
                ...
                .child(
                    h_flex()
                        ...
                        .child(Button::new("refresh-agents").with_size(Size::Small).ghost().icon(Icon::new(IconName::Redo2).with_size(Size::Small)).on_click(refresh)),
                ),
        )
        .child(
            v_flex()
                ...
                .children(state.agents.iter().map(|agent| {
                    let selected = state.selected_agent_id.as_ref().is_some_and(|id| id == &agent.id);
                    ...
                    v_flex()
                        ...
                        .border_1()
                        .border_color(rgb(0x27303a))
                        .child(h_flex().items_center().justify_between().child(div().child(agent.name.clone())).child(Button::new(launch_id).with_size(Size::Small).primary().label("Launch").on_click(cx.listener({
                            let agent_id = agent.id.clone();
                            move |state, _, window, cx| state.launch_session(agent_id.clone(), window, cx)
                        }))))
                        .child(div().text_size(px(12.)).text_color(rgb(0x9ca3af)).child(agent.id.clone()))
